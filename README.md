...
Included in the verilog files are two modules, transmitter and receiver. Both modules take baud rate, parity type, and number of stop bits as parameters. They are capable of transmitting 1 byte/8 bits of data per packet. Both modules take a 50MHz clock as input and work on the rising edge, clock speed must be changed in the code itself.
...
## Example Usage:
I was using a **DE10-Lite board** with an Altera MAX 10 FPGA on it for running and testing.The following is my example testing code with GPIO 0 and GPIO 1 connected with a jumper cable.
